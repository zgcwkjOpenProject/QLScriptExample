...
# Author: Kirin
# Last Modified by: zgcwkj
# Last Modified time: 2024-8-26 15:30:00
# sendNotify 推送通知功能
...
# 通知服务
NOTIFYAPI = ''  			# 自建接口服务（https://github.com/zgcwkjOpenProject/Typecho_Plugins_WeChatNotice）
							# http://baidu.com/wxn?apiToken=token&u=baidu.com&t={0}&c={1}
...
if "NOTIFYAPI" in os.environ and os.environ["NOTIFYAPI"]:
    NOTIFYAPI = os.environ["NOTIFYAPI"]
...
if NOTIFYAPI:
    notify_mode.append('NotifyApi')
    # print("NotifyApi 推送打开")
if BARK:
...
def notifyApi(title, content):
    print("\n")
    try:
        response = requests.get(NOTIFYAPI.format(title,urllib.parse.quote_plus(content))).json()
...
def send(title, content):
    ...
    for i in notify_mode:
        if i == 'NotifyApi':
            if NOTIFYAPI:
                notifyApi(title=title, content=content)
            else:
                print('未启用 NotifyApi')
            continue
        if i == 'bark':
