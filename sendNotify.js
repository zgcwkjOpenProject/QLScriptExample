...
// =======================================NotifyApi通知设置区域===========================================
// 自建接口服务（https://github.com/zgcwkjOpenProject/Typecho_Plugins_WeChatNotice）
...
// ==========================云端环境变量的判断与接收=========================
if (process.env.NOTIFYAPI) {
    NOTIFYAPI = process.env.NOTIFYAPI;
}
...
async function sendNotify(
  ...
  await Promise.all([
    NotifyApi(text, desp, params), // NotifyApi
...
function NotifyApi(text, desp, params = {}) {
  return new Promise((resolve) => {
    if (NOTIFYAPI) {
      let url = NOTIFYAPI.format(text, encodeURIComponent(desp));
      const options = {
        url: url,
        headers: {
